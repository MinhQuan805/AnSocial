  ...
  if (!hasOAuthConnection) {
    return (
      <Alert className="border-amber-300/80 bg-amber-50 text-amber-900">
        ...
        <AlertDescription>
          Connect your Instagram account first. Builder and HTTP Request linking only work after
          OAuth is connected.
        </AlertDescription>
      </Alert>
    );
  ...
  if (!isInstagramOAuthLinked) {
    return (
      <Alert className="border-amber-300/80 bg-amber-50 text-amber-900">
        ...
        <AlertDescription>
          Instagram Builder is currently detached from HTTP Request. Change Authorization mode to
          OAuth to link them.
        </AlertDescription>
      </Alert>
    );
