...
export function IntegrationCatalogPage() {
  ...
  const connectProvider = async (item: IntegrationCatalogItem) => {
    ...
    if (!item.providerType) {
      setConnectStatus("This integration currently uses manual setup. Follow the Connect Guide below.");
      return;
    }
    ...
    try {
      ...
      setConnectError(error instanceof Error ? error.message : "Unable to connect this integration.");
    } finally {
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <header className="space-y-3 text-center">
        ...
        <p className="mx-auto max-w-4xl text-pretty text-sm leading-relaxed text-muted-foreground md:text-lg">
          Keep all your marketing data in one place with Notion. Import social media insights,
          track campaign performance, and organize analytics from LinkedIn, TikTok, and more without coding.
        </p>
      </header>
                  ...
                  <CardHeader className="space-y-3 pb-2">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        <CardTitle className="text-3xl leading-tight md:text-2xl">{item.title}</CardTitle>
