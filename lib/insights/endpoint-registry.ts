...
const IG_USER_FIELDS: FieldOption[] = [
  ...
  { key: "profile_picture_url", label: "Profile Picture URL", description: "URL of profile picture.", group: "BASIC" },
  { key: "has_profile_pic", label: "Has Profile Picture", description: "Whether the account has a profile picture.", group: "BASIC" },
  { key: "is_published", label: "Is Published", description: "Whether the Instagram account is published.", group: "BASIC" },
  { key: "followers_count", label: "Followers Count", description: "Total followers.", group: "STATS" },
  { key: "follows_count", label: "Follows Count", description: "Total accounts followed.", group: "STATS" },
  { key: "media_count", label: "Media Count", description: "Total media published.", group: "STATS" },
  { key: "shopping_product_tag_eligibility", label: "Shopping Tag Eligibility", description: "Whether shop tagging is available.", group: "COMMERCE" },
];
...
const IG_USER_INSIGHT_METRICS: MetricOption[] = [
  ...
  },
  {
    ...
    allowedTimeframes: ["this_week", "this_month", "last_14_days", "last_30_days", "last_90_days", "prev_month"],
...
const IG_USER_MEDIA_FIELDS: FieldOption[] = [
  ...
  { key: "media_type", label: "Media Type", description: "IMAGE, VIDEO, or CAROUSEL_ALBUM.", group: "BASIC" },
  ...
  { key: "thumbnail_url", label: "Thumbnail URL", description: "Thumbnail URL (video only).", group: "BASIC", mediaTypes: ["VIDEO", "REELS"] },
  { key: "timestamp", label: "Timestamp", description: "ISO 8601 creation timestamp.", group: "BASIC" },
  ...
  { key: "media_product_type", label: "Media Product Type", description: "AD, FEED, STORY or REELS.", group: "CLASSIFICATION" },
  { key: "is_comment_enabled", label: "Comments Enabled", description: "Whether comments are enabled.", group: "CLASSIFICATION" },
  { key: "is_shared_to_feed", label: "Shared To Feed", description: "Whether reel appears in feed tab.", group: "CLASSIFICATION" },
  { key: "comments_count", label: "Comments Count", description: "Number of comments.", group: "ENGAGEMENT" },
  { key: "like_count", label: "Like Count", description: "Number of likes.", group: "ENGAGEMENT" },
  { key: "alt_text", label: "Alt Text", description: "Alternative text for images.", group: "ACCESSIBILITY" },
];
...
const IG_USER_TAGS_FIELDS: FieldOption[] = [
  ...
  { key: "username", label: "Username", description: "Creator username.", group: "PUBLISHER" },
  { key: "comments_count", label: "Comments Count", description: "Comment count.", group: "ENGAGEMENT" },
...
const IG_MEDIA_FIELDS: FieldOption[] = [
  ...
  { key: "permalink", label: "Permalink", description: "Permanent URL.", group: "BASIC" },
  { key: "thumbnail_url", label: "Thumbnail URL", description: "Thumbnail for video.", group: "BASIC", mediaTypes: ["VIDEO", "REELS"] },
  { key: "timestamp", label: "Timestamp", description: "ISO 8601 creation timestamp.", group: "BASIC" },
  { key: "shortcode", label: "Shortcode", description: "Instagram shortcode.", group: "BASIC" },
  { key: "caption", label: "Caption", description: "Caption text. Not available for album children.", group: "CONTENT" },
  { key: "alt_text", label: "Alt Text", description: "Accessibility text.", group: "CONTENT", mediaTypes: ["IMAGE", "FEED"] },
  { key: "username", label: "Username", description: "Creator username.", group: "PUBLISHER" },
  { key: "owner", label: "Owner", description: "Owner object.", group: "PUBLISHER" },
  { key: "media_product_type", label: "Media Product Type", description: "AD, FEED, STORY, REELS.", group: "CLASSIFICATION" },
  { key: "is_comment_enabled", label: "Comments Enabled", description: "Comments on/off. Not for album children.", group: "CLASSIFICATION" },
  { key: "is_shared_to_feed", label: "Shared To Feed", description: "Reel appears in feed tab.", group: "CLASSIFICATION", mediaTypes: ["REELS"] },
  { key: "comments_count", label: "Comments Count", description: "Comment count.", group: "ENGAGEMENT" },
  { key: "like_count", label: "Like Count", description: "Like count.", group: "ENGAGEMENT" },
  { key: "view_count", label: "View Count", description: "Reel view count (organic + paid). Business Discovery only.", group: "ENGAGEMENT", mediaTypes: ["REELS"] },
  { key: "copyright_check_information{status}", label: "Copyright Check", description: "Copyright detection status.", group: "COPYRIGHT", mediaTypes: ["VIDEO", "REELS"] },
  { key: "boost_eligibility_info", label: "Boost Eligibility", description: "Ad boost eligibility info.", group: "ADS" },
  { key: "boost_ads_list", label: "Boost Ads List", description: "Related active ads overview.", group: "ADS" },
];
...
const IG_MEDIA_COMMENTS_FIELDS: FieldOption[] = [
  ...
  { key: "like_count", label: "Like Count", description: "Likes on this comment.", group: "ENGAGEMENT" },
  { key: "hidden", label: "Hidden", description: "Whether comment is hidden.", group: "MODERATION" },
  { key: "from", label: "From", description: "Object with commenter id and username.", group: "PUBLISHER" },
];
...
const IG_MEDIA_PRODUCT_TAGS_FIELDS: FieldOption[] = [
  ...
  { key: "x", label: "X Position", description: "Horizontal tag position (0.0-1.0).", group: "POSITION" },
  { key: "y", label: "Y Position", description: "Vertical tag position (0.0-1.0).", group: "POSITION" },
];
...
export function validateInsightSelection(
  ...
  const allowedPeriods = effectiveMetrics.length > 0
  ...
  const resolvedPeriod = period && allowedPeriods.includes(period)
    ? period
    : allowedPeriods[0] ?? "day";

  // Auto-resolve period silently — no warning needed since the UI auto-selects

  // Compute allowed timeframes
  const allowedTimeframes = group === "DEMOGRAPHIC"
    ? effectiveMetrics.reduce<string[]>((acc, m) => {
        const def = metricMap.get(m);
  ...
  if (group === "DEMOGRAPHIC") {
    resolvedTimeframe = timeframe && allowedTimeframes.includes(timeframe)
      ? timeframe
      : allowedTimeframes[0] ?? "this_week";
  }

  // Compute allowed breakdowns (intersection)
  const allowedBreakdowns = effectiveMetrics.length > 0
    ? effectiveMetrics.reduce<string[]>((acc, m) => {
        const def = metricMap.get(m);
        if (!def) return acc;
  ...
  const metricType = group === "DEMOGRAPHIC" || resolvedBreakdown || !allSupportTimeSeries
    ? "total_value"
    : "time_series";
 ...
 */
export function getFieldsForMediaType(
  fields: FieldOption[],
  mediaType?: string,
): FieldOption[] {
  if (!mediaType || mediaType === "ALL") return fields;
  return fields.filter(
    (f) => !f.mediaTypes || f.mediaTypes.includes(mediaType),
  );
}
...
export function getMetricsForMediaType(
  ...
  return metrics.filter(
    (m) => !m.mediaTypes || m.mediaTypes.includes(mediaType),
  );
}
