...
export function resolveInsightRequest(args: {
  ...
  const breakdown = args.breakdown && allowedBreakdowns.includes(args.breakdown) ? args.breakdown : undefined;
  ...
  const supportsTimeSeries =
    ...
    effectiveMetrics.every((metric) => METRIC_RULES[metric].allowedMetricTypes.includes("time_series"));
