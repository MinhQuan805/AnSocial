...
import type { IProviderConnectionRepository, ProviderConnectionPayload } from "@/lib/core/auth.types";
...
export class FacebookProvider extends BaseProvider {
  ...
  public async refreshToken(connection: any): Promise<any> {
    const newToken = await this.facebookRepo.exchangeShortForLongToken(
